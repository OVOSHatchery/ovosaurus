...
scores = {}
try:
    with open(f"{dirname(dirname(__file__))}/pretrained/accuracy_nb.json", "r") as _f:
        scores = json.load(_f)
except:
    pass
...
for langs in lang_combos:
    ...
    name = f"nb_{clf_pipeline}_{'_'.join(langs)}"
    ...
    print(score)
    scores[name] = score
    with open(f"{dirname(dirname(__file__))}/pretrained/accuracy_nb.json", "w") as _f:
