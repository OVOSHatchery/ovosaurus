...
scores = {}
try:
    with open(f"{dirname(dirname(__file__))}/pretrained/accuracy_dtree.json", "r") as _f:
        scores = json.load(_f)
except:
    pass
...
for langs in lang_combos:
    ...
    name = f"dtree_{clf_pipeline}_{'_'.join(langs)}"
    ...
    print(score)
    scores[name] = score
    with open(f"{dirname(dirname(__file__))}/pretrained/accuracy_dtree.json", "w") as _f:
